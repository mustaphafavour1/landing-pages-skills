...
**Second revision, same day:** display typeface swapped from Bricolage Grotesque to Alan Sans on direct request (no reason given beyond preference — not a correction of a mistake, just a straight substitution). `typeface-library.md` updated: Alan Sans added as a new active display entry, Bricolage Grotesque's headfavour.com usage note updated to reflect the swap rather than deleted. Build produced one build-time warning worth knowing about ahead of time next time a newer Google Font gets proposed: `next build` (Turbopack) logs "Failed to find font override values for font `Alan Sans` — skipping generating a fallback font" — this only means Next.js doesn't have precomputed fallback-metrics for that particular family yet, so its usual layout-shift-reduction trick doesn't apply; the font still loads and renders correctly, nothing to fix. Worth a quick mention to the person when it comes up, not a blocker.

**Third revision, later:** the person changed the requirement itself — "I don't want it to be scrollable, just one non-scrolling page." This isn't a tweak on the same scrolling one-pager; it's a different layout mode, so most of the site's structure changed: Hero, Bio, and Socials merged into one compact identity column (a project list is the thing worth most of the space on a fixed viewport, not five separate full-height sections), the shipping-log rows got denser, and every scroll-triggered `whileInView` reveal switched to mount-triggered `animate` since nothing is off-screen anymore. Also swapped the project list to a new set the person provided (Design System added, Findanyrev dropped). New general technique captured in `taste-rules.md` rather than just here, since it's reusable well beyond this project: `h-dvh` + `overflow-hidden` down the container chain with `min-h-0` at every flexed level, `flex-1` rows so a list always exactly fills remaining space with no manual sizing, viewport-height-relative `clamp()` for graceful compression on short viewports, and — the part worth remembering most — verifying the actual requirement (`scrollHeight <= clientHeight`) scripted across several real viewport sizes rather than trusting a screenshot glance. Checked 8 sizes from 1920×1080 down to 375×667 plus an 844×390 landscape case; all passed cleanly.
